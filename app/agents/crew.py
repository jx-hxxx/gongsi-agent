...
def summarize_text(text: str, label: str = "") -> str:
    ...
    agent = Agent(
        role="공시 요약 전문가",
        goal="주어진 공시 본문의 실제 내용을 구체적으로 서술 요약한다. 사업 부문·제품·전략·정책 등 실질 사실을 직접 적되, 정밀 재무 수치의 망라적 나열은 하지 않는다(정확한 수치는 별도 검색이 담당).",
        backstory="너는 금융감독원 전자공시를 오래 분석해 온 애널리스트다. 근거에 없는 내용은 지어내지 않는다. '이 부분은 ~에 관한 내용이다' 식의 목차·메타 설명이 아니라, 실제 내용(어떤 사업·제품·정책인지)을 구체적으로 풀어 쓴다.",
    ...
    task = Task(
        description=(
            f"다음 공시 {head}본문의 실제 내용을 한국어 3~6문장으로 구체적으로 요약하라. "
            "**메타 설명 금지**: '이 부분은 ~에 관한 정보를 포함/제공한다', '~가 정리되어 있다' 같은 목차식 묘사를 쓰지 말고, "
            "실제 사실을 직접 서술하라(예: 어떤 사업 부문·제품·서비스·전략·정책·계약인지 구체적으로). "
            "정밀 재무 수치(금액·주식수 등)의 망라적 나열은 하지 마라 — 구체 숫자는 별도 검색(RAG)이 담당하므로 "
            "사업 이해에 꼭 필요한 대표 수치 1~2개 외에는 생략한다. 근거에 없는 추측·해석은 금지한다.\n\n"
            "=== 본문 ===\n{body}"
        ),
